...
pub fn run_backup(
    ...
    let prior = fs::read_to_string(out_dir.join(MARKER))
        .ok()
        ...
        for (_slug, src) in &project_dirs {
            ...
            for jsonl in transcripts_in(&tdir) {
                ...
                for r in crate::media::extract_media_refs(&parsed, &root) {
                    ...
                    if dest.exists() {
                        // Already captured on a prior run: reuse its recorded
                        // hash (avoids rehashing potentially huge media).
    ...
    *pbytes += len;
    ...
    if let Ok(dmeta) = fs::metadata(dest) {
        ...
        if same_len && same_mtime {
            ...
            maybe_hash_transcript(src, dest, slug, transcript_hashes);
            return Ok(());
        }
    }
...
fn maybe_hash_transcript(src: &Path, dest: &Path, slug: &str, out: &mut Vec<serde_json::Value>) {
